...
const Chat = (props) => {
  ...
  let handleFalse = () => {
    ind.forEach((i)=> {
      if(typeof i === 'object' && i._children) i.setNativeProps({style:{transform:[{scale:0}]}})
              ...
              <FlatList 
                ...
                onContentSizeChange={()=>{
                  let int = (async()=>{
                    ...
                    console.log(allRoom.get(props.route.params.roomNumber)[allRoom.get(props.route.params.roomNumber).length - 1].sender.name)

                    let loc = await localStorage.getItem(props.route.params.roomNumber + "number")
                    let parse = JSON.parse(loc)
                    if(name != allRoom.get(props.route.params.roomNumber)[allRoom.get(props.route.params.roomNumber).length - 1].sender.name){
                    if( parse < allRoom.get(props.route.params.roomNumber)[allRoom.get(props.route.params.roomNumber).length - 1].number
                    || msgLength.get(props.route.params.roomNumber).length < allRoom.get(props.route.params.roomNumber).length
                    ){
                      down2 = 1 
                      localStorage.setItem(props.route.params.roomNumber + 'end', '' ).then(()=>{})
                                ...
                                <_.div style={{ flexDirection: item.sender.name === name ? 'row-reverse' : 'row' }}>
                                  <View style={{ position: 'relative' }} >
                                    ...
                                    onPress={()=> {
                                      ind.push(index)
                                      handleFalse()
                                      if(typeof index === 'object' && index._children) index.setNativeProps({style:{transform:[{scale:1}]}})           
                                    }} show={first} >
                                      {item.sender.name === name ?
                                        <>
                                          <Text style={{ textAlign: 'center', zIndex: 1,flexGrow:1, backgroundColor:'#bbc', padding:7 }} onPress={() => {  oneDeleteClick(item.id); handleFalse(); }} > برای شما</Text>
                                          <View style={{ borderTopWidth: 1, marginVertical:7 }} />
                                          <Text style={{ textAlign: 'center', zIndex: 1,flexGrow:1, backgroundColor:'#bbc', padding:7 }} onPress={() => {  msgDeleteClick(item.id); handleFalse(); }} >برای همه</Text>
                                        </> :
                                        <>
                                          <Text style={{ textAlign: 'center', zIndex: 1,flexGrow:1, backgroundColor:'#bbc', padding:7 }} onPress={() => { oneDeleteClick(item.id); handleFalse(); }} > حذف </Text>
                                        </>}
                                    </Dropdown>
                                  </View>
                                </_.div>
        ...
        <_.div className={s.shadow} style={{ borderRadius: 5, width: '91%', alignSelf: 'center' }}>
          <_.div style={{ minWidth: '100%' }} >
            <View style={{ position: 'absolute', top: 2.5, right: '15%', zIndex: 111 }}>
              <Dropdown ref={(e)=> this.dr = e} onPress={()=>{ handleFalse(); this.dr && this.dr.setNativeProps({style:{transform:[{scale:1}] }}) }} top icon='paperclip' showBgcolor="#888" icon2False show={first} >
